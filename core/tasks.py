    ...
    for device in devices:
        ...
        if currently_offline and device.is_offline and not device.offline_notified:
            ...
            notify_telegram(
                device.client,
                f"🔴 <b>{company}</b>\n\n"
                f"<b>{device.display_name}</b> dejó de reportar telemetría.\n"
                f"Último contacto: {last_seen_local.strftime('%d/%m/%Y %H:%M') if last_seen_local else '—'} "
                f"({minutes:.0f} min)"
            )
        ...
        # ── Caso 3: estaba offline, ahora volvió ──────────────────────────
            ...
            if offline_since:
                notify_telegram(
                    device.client,
                    f"🟢 <b>{company}</b>\n\n"
                    f"<b>{device.display_name}</b> volvió a reportar telemetría.\n"
                    f"Estuvo sin conexión {duration_txt}."
                )
...
@shared_task(name="core.purge_old_telemetry")
def purge_old_telemetry():
    """
    Elimina los TelemetrySnapshot más antiguos que SENTINEL_TELEMETRY_RETENTION_DAYS
    (por defecto 30 días) en todos los dispositivos.

    Antes esta limpieza se ejecutaba dentro del endpoint de ingestión, en cada
    POST del agente (cada 5s por equipo) — miles de DELETEs redundantes por hora
    en el camino crítico de la petición. Ahora corre una vez al día vía Celery Beat.
    ...
    deleted, _ = TelemetrySnapshot.objects.filter(captured_at__lt=cutoff).delete()

    if deleted:
        logger.info(f"purge_old_telemetry: {deleted} snapshot(s) eliminados "
